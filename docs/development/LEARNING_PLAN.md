...
P6 学习产物以磁盘重启测试为核心证据，而不是只看类名：先关闭旧连接，再创建新 resources/Graph/Service/model/lock；对照完整 checkpoint messages 与瞬时 model messages；故障注入版本缺失、协议拆分、重复 event key 和跨库部分清理。六段复写总量建议 350 行，详见 P6 学习笔记。
