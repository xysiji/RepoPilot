...
| 记录 | 定向参考 | 实际处理 | 复制结论 | 验证 |
| --- | --- | --- | --- | --- |
| P6-CR-020 | Session model/store/manager | 使用官方 AsyncSqliteSaver；应用只新增安全 run 索引 | 未复制参考源码 | 两个磁盘重启场景 |
| P6-CR-013/014 | Trace record/writer | 独立设计 allowlist payload、event key 与 SQLite 表 | 未复制 EventBus/Writer | replay 唯一键与 API 测试 |
| P6-CR-022/023 | budget/compactor | 只采用“预算、工具协议、原 State 不变”的问题边界 | 未复制 compactor；未使用摘要 LLM | ContextManager 单测与 Demo stats |

P6 定向阅读了索引列出的 Session、Trace、Context 文件和对应测试，没有扫描或修改全部 `reference_materials/`。第三方 LangGraph/aiosqlite 代码通过依赖使用，未复制到仓库。
