...
本文件记录候选复用与已执行结论。P0 已实现基础设施，P1 手写循环已在 P2 被 StateGraph 替换；P3 在不改变 Graph 拓扑的前提下加入安全执行器、静态 Policy、统一 Envelope 与失败分类。没有从 KamaClaude 复制产品代码。
...
| CR-008 | `src/kama_claude/core/tools/invocation.py` | `tools/executor.py` + `agent/nodes.py` | `adapted` | 参数先校验、失败回填、ID 配对 | P3 独立实现严格管线；不发 EventBus、不重试、不审批、不泄漏异常 |
