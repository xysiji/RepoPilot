...
本文件记录候选复用与已执行结论。P0 已实现配置、模型工厂和 FastAPI Application Factory；P1 的手写 Tool Calling 循环已在 P2 被显式 LangGraph StateGraph 替换，三个只读工具继续复用。没有从 KamaClaude 复制产品代码，所有实现均针对当前框架与测试重新编写。
...
| CR-005 | `src/kama_claude/core/context.py` | `agent/state.py` | `framework-replaced` | messages 与控制状态分离、追加语义 | P2 定义最小 reducers；Patch/审批/测试字段仍未实现 |
...
| CR-008 | `src/kama_claude/core/tools/invocation.py` | `agent/nodes.py::ToolNode` 错误边界 | `adapted` | 参数错误、未知工具、异常与结构化失败回填 | 不发 EventBus、不做超时/重试/审批；每个调用生成稳定 ToolMessage |
