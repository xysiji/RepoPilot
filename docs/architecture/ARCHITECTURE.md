...
P3 的唯一生产执行引擎仍是无 Checkpointer 的编译 `StateGraph`。P2 拓扑未改变；变化集中在 tools 节点内部的确定性安全执行管线。它仍不包含规划、写入、审批、测试反馈、持久化或流式输出。
...
- `ToolSafetyPolicy` 以 Python mapping 把三个生产工具固定为 `read_only`；write/command/unknown effect 一律 fail closed，P3 不创建审批。
...
对应实现位于 `src/repopilot/tools/contracts.py`、`policy.py`、`executor.py`、`readonly.py`，以及 `agent/nodes.py` 和 `services/agent_service.py`。P1 手写循环只保留在 Git 历史基线中。
...
参数格式错误属于给 Agent 的纠错信息，必须在审批前返回；用户不应该收到一张参数本身就非法的审批卡。P3 已实现到只读执行与分类：只读工具在策略判断后执行，其他 effect 在执行前拒绝。`apply_patch`、`run_tests` 和人工审批尚未实现。失败信息只有在限长、分类后才进入下一轮模型上下文。
